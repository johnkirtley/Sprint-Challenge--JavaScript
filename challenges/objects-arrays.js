...
const universities = graduates.map(i => i.university).sort();
...
const substring = 'uni';
const result = substring => graduates.filter(({
  university
}) => university.toLowerCase().includes(substring));
const unisWithUni = result(substring);
...
*/
const populationTotal = zooAnimals.reduce((accum, value) => accum + value.population, 0)
